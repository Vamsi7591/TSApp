...
export class WeeklyPage {
  ...
  ngAfterViewInit() {
    ...
    this.auth.loadWeeklyReport()
      .then(data => {
        this.weekDetails = data;
        for (let k = 0; k < this.weekDetails.length; k++) {
          this.pieChartLabels.push(this.weekDetails[k].projectName);
          this.pieChartData.push(this.weekDetails[k].duration);
        }
        this.loadPie();
      });
  }
